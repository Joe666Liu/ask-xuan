...
Ask Xuan uses [better-auth](https://www.better-auth.com/) as its authentication framework, with session data stored in PostgreSQL. Authentication is optional — if `DATABASE_URL` and `BETTER_AUTH_SECRET` are not configured, the app runs in static mode without user login.
...
Ask Xuan supports GitHub and Google as OAuth providers. They are automatically enabled when the corresponding environment variables are set.
...
Ask Xuan supports [Cloudflare Turnstile](https://developers.cloudflare.com/turnstile/) to protect sign-in, sign-up, and OAuth flows from bots.
