...
Ask Xuan 使用 [better-auth](https://www.better-auth.com/) 作为认证框架，会话数据存储在 PostgreSQL 中。认证是可选的——如果未配置 `DATABASE_URL` 和 `BETTER_AUTH_SECRET`，应用将以静态模式运行，不包含用户登录功能。
...
Ask Xuan 支持 GitHub 和 Google 作为 OAuth 登录方式。配置对应的环境变量后会自动启用。
...
Ask Xuan 支持 [Cloudflare Turnstile](https://developers.cloudflare.com/turnstile/) 来保护登录、注册和 OAuth 流程，防止机器人攻击。
