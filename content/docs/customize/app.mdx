...
## Basic Information
Ask Xuan's basic application information is configured in the `src/config/site-config.ts` file.

```ts title="src/config/site-config.ts"
export const siteConfig = {
  title: "Ask Xuan",
  author: "Jayden & Lingjue",
  description: "Ask Xuan ",
  keywords: [],
  theme: {
...
Ask Xuan includes built-in privacy policy and terms of service pages. Content is stored in the `content/legal/` directory:
...
You don't need to configure the sitemap manually — Ask Xuan automatically generates it. This is a built-in capability of TanStack Start. See the [TanStack Start documentation](https://tanstack.com/start/latest/docs/framework/react/guide/seo#sitemaps) for more.

## Robots.txt
Robots.txt tells search engines which pages can be crawled and which cannot.

Ask Xuan's robots.txt configuration is centralized in the `src/config/robots-config.ts` file. Ask Xuan automatically generates the robots.txt file.
