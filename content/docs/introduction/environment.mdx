...
Ask Xuan supports two ways to configure environment variables:
1. Via environment variable files — for values that rarely change, such as database connection URLs and auth secrets.
2. Via the admin config panel — for values that change frequently. Ask Xuan provides a dynamic config panel where you can modify variables at any time, such as payment keys and email server settings.
...
| Variable   | Description |
| -------- | ------- |
| DATABASE_URL  | Runtime database connection URL. For Supabase, use the pooled transaction URL. |
| DATABASE_MIGRATION_URL  | Optional migration connection URL. For Supabase, prefer the direct URL for Drizzle migrations. |

### Authentication

Ask Xuan uses Supabase Auth. See the authentication setup documentation for more detailed instructions.

| Variable   | Description |
| -------- | ------- |
| VITE_SUPABASE_URL  |  Supabase project URL    |
| VITE_SUPABASE_ANON_KEY  |  Supabase publishable/anon key for browser auth    |
| ADMIN_EMAILS  |  Admin email addresses    |

Configure OAuth providers such as GitHub and Google in the Supabase Dashboard. No provider secrets are required in this app.

### TURNSTILE
Ask Xuan uses TURNSTILE as a captcha service to protect your website. You need to register an account on the [TURNSTILE](https://developers.cloudflare.com/turnstile/?preferred-color-scheme=light) website and create a site to get the site key. See the captcha setup documentation for more details.
...
### Email
You can use the Resend service or a custom email server. Ask Xuan recommends using Resend. See the email setup documentation for more details.
...
| EMAIL_FROM  | Sender email, e.g.: `Ask Xuan <noreply@auth.askxuan.dev>`    |
...
### Credits
To simplify developer configuration, Ask Xuan integrates the credit system into the admin panel. You can modify credit settings at any time in the admin config panel, such as credit amounts and expiration periods. See the credit setup documentation for more details.
...
Ask Xuan provides a dynamic config panel where you can modify the following settings at any time without restarting the service.
...
Ask Xuan supports S3-compatible storage services (e.g., AWS S3, Cloudflare R2, MinIO).
